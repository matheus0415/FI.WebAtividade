$(document).ready(function () {
  $("#formCadastro").submit(function (e) {
    e.preventDefault();

    var dadosBeneficiarios = [];

    var beneficiariosArray = window.beneficiarios || beneficiarios || [];

    if (beneficiariosArray && beneficiariosArray.length > 0) {
      dadosBeneficiarios = beneficiariosArray.map(function (beneficiario) {
        return {
          Nome: beneficiario.nome,
          CPF: beneficiario.cpfLimpo,
        };
      });
    }

    var dadosParaEnvio = {
      NOME: $(this).find("#Nome").val(),
      CEP: $(this).find("#CEP").val(),
      Email: $(this).find("#Email").val(),
      Sobrenome: $(this).find("#Sobrenome").val(),
      Nacionalidade: $(this).find("#Nacionalidade").val(),
      Estado: $(this).find("#Estado").val(),
      Cidade: $(this).find("#Cidade").val(),
      Logradouro: $(this).find("#Logradouro").val(),
      Telefone: $(this).find("#Telefone").val(),
      CPF: $(this).find("#CPF").val().replace(/\D/g, ""),
      Beneficiarios: dadosBeneficiarios,
    };

    $.ajax({
      url: urlPost,
      method: "POST",
      data: dadosParaEnvio,
      error: function (r) {
        if (r.status == 400) ModalDialog("Ocorreu um erro", r.responseJSON);
        else if (r.status == 500)
          ModalDialog(
            "Ocorreu um erro",
            "Ocorreu um erro interno no servidor."
          );
      },
      success: function (r) {
        ModalDialog("Sucesso!", r);
        $("#formCadastro")[0].reset();

        if (typeof beneficiarios !== "undefined") {
          beneficiarios = [];
          contadorBeneficiarios = 0;
          if (typeof atualizarTabelaBeneficiarios === "function") {
            atualizarTabelaBeneficiarios();
          }
          if (typeof atualizarListaPrincipal === "function") {
            atualizarListaPrincipal();
          }
        }
      },
    });
  });
});

function ModalDialog(titulo, texto) {
  var random = Math.random().toString().replace(".", "");
  var texto =
    '<div id="' +
    random +
    '" class="modal fade">                                                               ' +
    '        <div class="modal-dialog">                                                                                 ' +
    '            <div class="modal-content">                                                                            ' +
    '                <div class="modal-header">                                                                         ' +
    '                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>         ' +
    '                    <h4 class="modal-title">' +
    titulo +
    "</h4>                                                    " +
    "                </div>                                                                                             " +
    '                <div class="modal-body">                                                                           ' +
    "                    <p>" +
    texto +
    "</p>                                                                           " +
    "                </div>                                                                                             " +
    '                <div class="modal-footer">                                                                         ' +
    '                    <button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button>             ' +
    "                                                                                                                   " +
    "                </div>                                                                                             " +
    "            </div><!-- /.modal-content -->                                                                         " +
    "  </div><!-- /.modal-dialog -->                                                                                    " +
    "</div> <!-- /.modal -->                                                                                        ";

  $("body").append(texto);
  $("#" + random).modal("show");
}
